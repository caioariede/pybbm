...
class BaseCategoryView(RedirectToLoginMixin, generic.DetailView):
...
class BaseForumView(RedirectToLoginMixin, PaginatorMixin, generic.ListView):
...
class BaseTopicView(RedirectToLoginMixin, PaginatorMixin, generic.ListView):
...
class BaseAddPostView(PostEditMixin, generic.CreateView):
...
class BaseEditPostView(PostEditMixin, generic.UpdateView):
...
class BaseDeletePostView(generic.DeleteView):
...
class BaseTopicPollVoteView(generic.UpdateView):
    model = Topic
